...
class ConfigurationAccountantsController extends Controller
{
    ...
    /*public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'numPlaca' => [
                ...
                function ($attribute, $value, $fail) {
                    ...
                    $numPlaca = collect($value)->first();

                    // Verificar si ya existe en la base de datos con una máquina distinta
                    $existe = Acumulado::where('NumPlaca', $numPlaca)->exists();

                    if ($existe) {
                        $fail("El número de placa $numPlaca ya está asociado a otra máquina.");
                    }
                },
            ],
        ]);
        ...
        $machineId = $request->machine_id;
        $numPlaca = collect($request->numPlaca)->first(); // Extraer el primer valor del array
        $alias = collect($request->alias)->first(); // Extraer el primer valor del array
        $local = Local::first();
        // Verificar si ya existe el registro en la base de datos
        $existe = Acumulado::where('machine_id', $machineId)
            ->where('NumPlaca', $numPlaca)
            ->exists();

        if ($existe) {
            return response()->json([
                'message' => 'Ya existe un registro con esta combinación de Machine ID y NumPlaca.',
            ], 409);
        }

        // Obtener datos de la tabla acumulado en ComData usando la conexión externa
        $conexionComdata = nuevaConexionLocal('admin');

        if (!$conexionComdata) {
            return response()->json([
                'message' => 'Error de conexión con ComData.',
            ], 500);
        }
        ...
        if (!$datosAcumulado) {
            return response()->json([
                'message' => 'No se encontraron datos en la tabla acumulado para este NumPlaca.',
            ], 404);
        }
        ...
        $acumulado = Acumulado::create([
            ...
            'local_id' => $local->id,
            'machine_id' => $machineId,
            'nombre' => $alias, // Usamos el alias como nombre
            'entradas' => $datosAcumulado->entradas ?? 0,
            'salidas' => $datosAcumulado->salidas ?? 0,
            'CEntradas' => $datosAcumulado->CEntradas ?? 0,
            'CSalidas' => $datosAcumulado->CSalidas ?? 0,
